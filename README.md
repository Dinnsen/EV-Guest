...
<p align="center">
  <img src="https://raw.githubusercontent.com/Dinnsen/EV-Guest/main/docs/assets/EV-Guest.jpg" width="420" alt="EV Guest logo">
</p>
...
Version **0.6.1** keeps the simple v0.4.0/v0.5.1 entity model and adds the fixed config flow from v0.6.0 without changing existing entity IDs or names.
...
- **Optional Charger Switch Entity** (`switch.*`) for direct start/stop charger control using the same simple Model A approach as v0.4.0
- **Optional Charger Status Entity** (`switch.*`, `binary_sensor.*`, or `sensor.*`) if you want EV Guest to read charger state
- **Name**
...
- Plate provider
- MotorAPI API key
- Optional Charger Switch Entity
- Optional Charger Status Entity
