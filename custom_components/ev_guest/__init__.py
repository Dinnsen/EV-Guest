...
async def async_migrate_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Ensure required keys exist so options and setup stay stable after upgrades.
    ...
    hass.config_entries.async_update_entry(entry, data=new_data, version=4, minor_version=0)
    _LOGGER.info("Migrated EV Guest config entry %s to version 4.0", entry.entry_id)
    return True
